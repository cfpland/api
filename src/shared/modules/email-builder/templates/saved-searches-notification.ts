export const savedSearchesNotificationTemplate = `<mjml>
  <mj-head>
    <mj-attributes>
  ...
  <mj-body>
    ...
    <mj-section>
      <mj-column>
        ...
        <mj-text line-height="20px">
          {{searches.length}} of your <a href="https://www.cfpland.com/c/searches/">saved searches</a> have new CFPs:
        </mj-text>
      </mj-column>
    </mj-section>
    {{#each searches.items}}
    ...
    <mj-section border-bottom="1px dashed lightgrey">
      <mj-column>
        <mj-text font-size="20px" padding-top="10px">
          {{ this.conferences.total }} New CFPs:
          {{#if this.search.options.category}}{{ capitalize this.search.options.category }}{{/if}}{{#and this.search.options.category this.search.options.region }}, {{/and}}{{#if this.search.options.region}}{{ capitalize this.search.options.region }}{{/if}}
        </mj-text>
      </mj-column>
    </mj-section>
    
    <!-- CFPs -->
    {{#each this.conferences.items}}
    <!-- CFP -->
    <mj-section border-bottom="1px dashed lightgrey">
      <mj-column>
    ...
    <mj-section>
      <mj-column>
        <mj-text align="center" padding="20px">
          <a class="see-more-cta" href="https://www.cfpland.com/c/searches/">
            View All Your Saved Searches
          </a>
          <p style="line-height: 20px">These notifications are sent when new CFPs are found matching your saved search.</p>
        </mj-text>
      </mj-column>
    </mj-section>
    <!-- Footer -->
